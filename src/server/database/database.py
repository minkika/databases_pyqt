import datetime
from sqlalchemy import Column, create_engine, DateTime, ForeignKey, Integer, MetaData, String, Table, Text
...
class ServerStorage:
    ...
    def process_message(self, sender, recipient):
        sender = self.session \
            .query(self.AllUsers) \
            .filter_by(name=sender) \
            .first().id
        recipient = self.session \
            .query(self.AllUsers) \
            .filter_by(name=recipient) \
            .first().id
        sender_row = self.session \
            .query(self.UsersHistory) \
            .filter_by(user=sender) \
            .first()
        sender_row.sent += 1
        recipient_row = self.session \
            .query(self.UsersHistory) \
